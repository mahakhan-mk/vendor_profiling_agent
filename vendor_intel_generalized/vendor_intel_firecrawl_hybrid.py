...
class VendorIntelConfig:
    ...
    enable_search_fallbacks: bool = os.getenv("ENABLE_SEARCH_FALLBACKS", "true").lower() not in {"0", "false", "no"}
    max_domain_hint_searches_per_query: int = int(os.getenv("MAX_DOMAIN_HINT_SEARCHES_PER_QUERY", "9"))
...
def _extract_firecrawl_results(body: Dict[str, Any]) -> List[Dict[str, Any]]:
    data = body.get("data", {})
...
def _search_once(config: VendorIntelConfig, payload: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for item in _extract_firecrawl_results(body):
...
def firecrawl_search(config: VendorIntelConfig, query: str) -> List[Dict[str, str]]:
    """
    Firecrawl-compatible hybrid search.

    Primary path uses documented includeDomains with broad natural-language queries.
    Some self-hosted or upstream Firecrawl deployments return zero results for includeDomains
    even when the target site has matching articles. For reliability, this function falls back to:
      1. broad web/news search without includeDomains, then local allowlist filtering;
      2. domain-hinted broad queries such as "Okta breach bleepingcomputer.com", still locally filtered.

    This keeps the system generalized. It does not hardcode one source and it never accepts
    ...
    primary_results = _search_once(config, primary_payload)
    all_results.extend(primary_results)

    if primary_results or not config.enable_search_fallbacks:
        return _dedupe_search_results(all_results)

    print("  includeDomains returned 0; trying broad search with local allowlist filter")
    broad_payload: Dict[str, Any] = {
        "query": query,
        "limit": config.max_search_results_per_query,
        "sources": config.search_sources,
    }
    broad_results = _search_once(config, broad_payload)
    all_results.extend(broad_results)

    if broad_results:
        return _dedupe_search_results(all_results)

    print("  broad search returned 0 approved results; trying domain-hinted searches")
    domains = list(EXTERNAL_INCIDENT_SOURCE_BONUS.keys())[: config.max_domain_hint_searches_per_query]
    per_domain_limit = max(3, min(8, config.max_search_results_per_query // 2))

    for domain in domains:
        hinted_payload: Dict[str, Any] = {
            "query": f"{query} {domain}",
            "limit": per_domain_limit,
            "sources": config.search_sources,
        }
        try:
            hinted_results = _search_once(config, hinted_payload)
        except Exception as exc:
            print(f"  domain-hinted search failed for {domain}: {exc}")
            continue
        all_results.extend(hinted_results)
