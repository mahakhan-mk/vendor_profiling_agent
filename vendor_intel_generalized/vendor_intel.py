...
class VendorIntelConfig:
    ...
    max_search_results_per_query: int = int(os.getenv("MAX_SEARCH_RESULTS_PER_QUERY", "10"))
...
def build_queries(vendor: str, product: Optional[str] = None) -> List[str]:
    ...
    vendor = vendor.strip()
    query_roots = [entity]
    if product and product.strip().lower() != vendor.lower():
        query_roots.append(vendor)
...
def firecrawl_search(config: VendorIntelConfig, query: str) -> List[Dict[str, str]]:
    response = requests.post(
        ...
        json={"query": query, "limit": config.max_search_results_per_query},
...
def discover_candidates(config: VendorIntelConfig, vendor: str, product: Optional[str]) -> List[Candidate]:
    ...
    for query in build_queries(vendor, product):
        print(f"\nSearching: {query}")
        try:
