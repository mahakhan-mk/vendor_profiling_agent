...
Goal:
Extract visible vendor evidence from this single approved page and finish immediately.

Strict rules:
- Do not browse outside the approved URL/domain.
- Do not click links.
- Do not search.
- Do not scroll unless the visible page text is clearly insufficient.
...
- Extract only factual observations directly supported by visible page text.
- Return evidence_items only for visible evidence.
- Do not create evidence_items for missing categories.
- Put missing or unvisited coverage only in limitations.
- Do not create evidence_items from generic navigation labels unless the label itself is clearly relevant.
- Return JSON only. No markdown. No prose outside JSON.

Confidence rules:
- Use "high" only when supporting_text directly and specifically supports the claim.
- Use "medium" when the page references the topic but lacks scope, date, detail, or linked evidence.
- Use "low" only when supporting_text is weak, generic, navigational, or only a link label.
- Do not mark all items as low by default.
...
{{
  ...
  "evidence_items": [
    {{
      ...
      "supporting_text": "exact or near-exact visible text from the page",
      "source_url": "{test_url}",
      "confidence": "high"
    }}
  ],
...
Finish as soon as the JSON can be produced.
"""
...
def save_result(result: BrowserEvidenceResult) -> None:
    ...
    output_path = output_dir / "browser_agent_max2.json"
